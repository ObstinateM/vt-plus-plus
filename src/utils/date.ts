...
export function getWeekNumber(event: Event, year?: number) {
    // const date = new Date();
    // date.setHours(0, 0, 0, 0);
    // date.setDate(date.getDate() + 3 - ((date.getDay() + 6) % 7));
    const week1 = new Date(year ?? new Date().getFullYear(), 0, 4);
...
export function getCurrentWeekNumber() {
    ...
    return (
        1 +
        Math.round(
            ((new Date().getTime() - week1.getTime()) / 86400000 - 3 + ((week1.getDay() + 6) % 7)) /
                7,
        )
    );
}
